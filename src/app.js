...
function validateRepositoryId(req, res, next) {
  ...
  if (!isUuid(id)) {
    return res.status(400).json({ error: "Invalid Id for repository" });
  }

  return next();
}

function existRepository(req, res, next) {
  const { id } = req.params;
...
app.use("/repositories/:id", validateRepositoryId, existRepository);
